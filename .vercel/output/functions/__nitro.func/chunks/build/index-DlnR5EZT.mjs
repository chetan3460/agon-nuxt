import { _ as __nuxt_component_0 } from './nuxt-link-C2i1f_CJ.mjs';
...
import { g as getImageUrl } from './getImageUrl-B0XNzhL2.mjs';
import { u as useFetch } from './fetch-5adf4HwG.mjs';
import { _ as _sfc_main$a } from './NuxtImg-C1SZL7yZ.mjs';
...
import { _ as _sfc_main$9 } from './BlogItem-DA6kinlY.mjs';
import { u as useDynamicComponents } from './useDynamicComponents-CoMJSdSL.mjs';
import 'unhead';
...
const _sfc_main$8 = {
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      ...
      { encode: false }
      // Prevent encoding for better readability
    );
    const { data, error } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/banner-block?${query}`, "$Z0s5OE3hU7")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (bannerBlock.value !== null) {
        ...
        _push(`</div></div><div class="hidden relative flex-1 h-auto self-stretch lg:block"><div class="absolute w-full left-20 animate-hero-thumb-sm-animation bottom-[-60px] max-w-[526px]"><img${ssrRenderAttr("src", unref(getImageUrl)((_b = (_a = bannerBlock.value) == null ? void 0 : _a.bannerImage) == null ? void 0 : _b.url))} class="h-full w-full object-cover" alt="image"></div><div class="absolute opacity-80 animate-float max-w-[176px] bottom-[10%] -translate-x-1/3"><div class="relative bg-green-900 bg-opacity-80 rounded-2xl grid place-items-center"><img class="h-full w-full object-cover"${ssrRenderAttr("src", unref(getImageUrl)((_c = bannerBlock.value.videoImg) == null ? void 0 : _c.url))} alt="Agon"><p class="text-heading-4 absolute text-white font-chivo font-bold ml-[49px]">Watch intro video </p><button class="rounded-full bg-white grid place-items-center absolute play-video w-[64px] h-[64px] left-[-15%]"><img${ssrRenderAttr("src", _imports_0$3)} alt="play button" width="15"></button></div></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
const _sfc_main$7 = {
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      {
        populate: {
          partnerItems: {
            populate: "Image"
            // Populate the Image field within partnerItems
          }
        }
      },
      { encode: false }
    ...
    const { data, error } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/partner-logo?${query}`, "$Cz1cmFR1CE")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
      ...
      if (partnerItems.value !== null) {
        ...
        ssrRenderList(partnerItems.value, (item, index) => {
          ...
          _push(ssrRenderComponent(_component_NuxtImg, {
            key: item.id,
            src: unref(getImageUrl)((_a2 = item.Image) == null ? void 0 : _a2.url),
...
const _sfc_main$6 = {
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      {
        populate: {
          SeeWhyBlockItems: {
            populate: "image"
            // Populate the image field within SeeWhyBlockItems
          }
        }
      },
      { encode: false }
    ...
    const { data, error } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/see-why-block?${query}`, "$nllH3o6E47")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
      seeWhyData.value = ((_a = data.value) == null ? void 0 : _a.data) || [];
    } else if (error.value) {
      console.error("Error fetching data:", error.value);
    } else {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (seeWhyData.value !== null) {
        ...
        ssrRenderList(seeWhyData.value.SeeWhyBlockItems, (item, index) => {
          var _a3;
          var _a2;
          _push(`<div class="${ssrRenderClass(["tab-content lg:gap-[30px] lg:flex", selectedTab.value === index ? index === 0 ? "bg-bg-2" : "bg-bg-3" : "!hidden"])}"><div class="p-5 md:p-[60px] lg:w-1/2"><div class="prose prose-h2:font-bold prose-h2:font-chivo prose-h2:text-[28px] prose-h2:leading-[32px] prose-h2:mb-[30px] prose-p:text-excerpt prose-p:text-[20px] prose-p:mb-[40px] mb-[60px]">${(_a3 = renderMarkdown(item.description)) != null ? _a3 : ""}</div><button type="button">`);
          _push(ssrRenderComponent(_component_NuxtLink, {
...
const _sfc_main$5 = {
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      {
        populate: {
          featureBlockItems: {
            populate: ["featureImg", "featureBackImg"]
    ...
    const { data, error, pending, refresh } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/feature-block?${query}`, "$hXrSNXdp3k")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (featureItemData.value !== null) {
        ...
        ssrRenderList(featureItemData.value.featureBlockItems, (item, index) => {
          ...
          _push(ssrRenderComponent(_component_NuxtImg, {
            src: unref(getImageUrl)((_a = item.featureImg) == null ? void 0 : _a.url),
          ...
          _push(`</button>`);
          _push(ssrRenderComponent(_component_NuxtImg, {
            src: unref(getImageUrl)((_b = item.featureBackImg) == null ? void 0 : _b.url),
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      {
        populate: {
          ...
          ListItems: "*"
          // Populate all fields within ListItems
        }
      },
      { encode: false }
      // Prevent encoding for better readability
    );
    const { data, pending, error, refresh } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/what-we-do-block?${query}`, "$QsRunil2nR")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (whatWeDoData.value !== null) {
        ...
        _push(ssrRenderComponent(_component_NuxtImg, {
          src: unref(getImageUrl)(whatWeDoData.value.Image.url),
    ...
    const query = qs.stringify(
      {
        populate: {
          SliderItems: {
            ...
            // Ensure only sliderImage is populated
          }
        }
      },
      { encode: false }
    );
    const { data, pending, error, refresh } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/what-we-offer-block?${query}`, "$lJQD9FIrwZ")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
                  ...
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    if (_push3) {
                      _push3(ssrRenderComponent(_component_NuxtImg, {
                        ...
                        src: unref(getImageUrl)(item.sliderImage.url),
...
const _sfc_main$2 = {
  ...
  async setup(__props) {
    ...
    const query = qs.stringify(
      {
        populate: {
          testimonialItems: {
            populate: ["image"]
            // Only populate the image field inside testimonialItems
          }
        }
      },
      { encode: false }
    ...
    const { data, pending, error, refresh } = ([__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/testimonial-block?${query}`, "$xcPIv5nwp2")), __temp = await __temp, __restore(), __temp);
    if (data.value) {
                  ...
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    ...
                    if (_push3) {
                      _push3(ssrRenderComponent(_component_NuxtImg, {
                        ...
                        src: unref(getImageUrl)((_a = item.image) == null ? void 0 : _a.url),
                      ...
                      return [
                        createVNode(_component_NuxtImg, {
                          class: "h-full w-full object-cover max-w-[55px] max-h-[55px] mb-[22px]",
                          src: unref(getImageUrl)((_b = item.image) == null ? void 0 : _b.url),
                          alt: item.image.alternativeText,
                          width: item.image.width || 50,
                          height: item.image.height || 50,
                          format: "webp"
...
const _sfc_main$1 = {
  ...
  async setup(__props) {
    ...
    const strapiBaseUrl = useNuxtApp().$strapiBaseUrl;
    const blogQuery = qs.stringify({ populate: "*" });
    const blogPostQuery = qs.stringify({ populate: "*" });
    [__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/blog-post?${blogQuery}`, "$caGyR0HCHE")), __temp = await __temp, __restore();
    [__temp, __restore] = withAsyncContext(() => useFetch(`${strapiBaseUrl}/api/blogs?${blogPostQuery}`, "$W3FJsJ7Iu9")), __temp = await __temp, __restore();
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NuxtLink = __nuxt_component_0;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "px-[12px] md:px-[36px] xl:px-0 mt-[70px]" }, _attrs))}>`);
...
//# sourceMappingURL=index-DlnR5EZT.mjs.map
